...
export default function App() {
  ...
  useEffect(() => {
      ...
      if (savedRole) {
          ...
          if (savedRole === 'admin' && SYSTEM_API_KEY) setApiKey(SYSTEM_API_KEY);
          else if (savedKey) setApiKey(savedKey);
      }
  }, []);

  const [products, setProducts] = useState<Product[]>(DEMO_DATA);
  const [userAvatar, setUserAvatar] = useState<string>(() => localStorage.getItem('tk_pro_avatar') || DEFAULT_AVATAR);
  ...
  const handleLogin = (input: string) => {
      ...
      if (trimmedInput === ADMIN_PASSWORD) enterSystem(SYSTEM_API_KEY, 'admin');
      else if (trimmedInput === USER_PASSWORD) enterSystem(localStorage.getItem('tk_pro_key') || "", 'user');
      else if (trimmedInput.startsWith("AIza")) enterSystem(trimmedInput, 'user');
      else alert("密码错误！");
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.name.endsWith('.csv')) {
        ...
        reader.onload = (evt) => processFileContent(parseCSV(evt.target?.result as string));
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(p => p.price >= priceRange[0] && p.price <= priceRange[1] && p.sales >= minSales);
  ...
  const handleProductSelect = (product: Product) => { setSelectedProduct(product); setIsAnalysisVisible(true); };
